...
class User
  ...
  def get_profile
    ...
    RestClient.get("#{BEANCOUNTER_HOST}/beancounter-platform/rest/user/#{username}/profile?token=#{token}") do |req, res, result|
  ...
  def get_user_data
    RestClient.get("#{BEANCOUNTER_HOST}/beancounter-platform/rest/user/#{username}/me?token=#{token}") do |req, res, result|
      if result.code == "200" && JSON.parse(req.body)["status"] == "OK"
